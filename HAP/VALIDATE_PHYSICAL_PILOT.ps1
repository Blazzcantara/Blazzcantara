param(
  ...
  [string]$OutputJson = ".\HAP\pilot\PILOT_PROMOTION_RECEIPT_v0.1.json",
...
if (-not (Test-Path $ResultsCsv)) { throw "Pilot results CSV not found: $ResultsCsv" }
...
$requiredIds = @("SHOW-ST01","SHOW-CV01","SHOW-SC01")
if ($rows.Count -ne $requiredIds.Count) {
  throw "Expected 3 pilot rows, found $($rows.Count)."
}

$ids = @($rows | ForEach-Object { $_.module_id })
if (@($ids | Sort-Object) -join "|" -ne @($requiredIds | Sort-Object) -join "|") {
  throw "Pilot module IDs do not match the required Straight / Curve / S-Curve set."
}
...
foreach ($row in $rows) {
  if ([int]$row.run_count -lt 10) {
    throw "$($row.module_id) requires at least 10 runs."
  }
...
$state = if ($AllowSyntheticCiFixture) {
  "SYNTHETIC_CI_PILOT_PASS_DO_NOT_PROMOTE"
}
else {
  "PHYSICAL_PILOT_PASS"
}
...
$receipt = [ordered]@{
  ...
  pilot_results_sha256 = $resultsHash
...
Write-Host "PASS: pilot promotion receipt generated" -ForegroundColor Green
